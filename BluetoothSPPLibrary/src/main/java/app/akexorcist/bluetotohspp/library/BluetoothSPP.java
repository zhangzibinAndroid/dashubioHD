...
public class BluetoothSPP {
    ...
    private boolean mSecure = true;//是否安全连接
    
    public BluetoothSPP(Context context, boolean secure) {
        mContext = context;
    ...
    public void connect(Intent data, boolean secure) {
    ...
    public void autoConnect(String keywordName, final boolean secure) {
        if(!isAutoConnectionEnabled) {
            keyword = keywordName;
                    ...
                    if(isServiceRunning) {
                        if(isAutoConnectionEnabled) {
                            ...
                            if(c >= arr_filter_address.size())
                                c = 0;
                            connect(arr_filter_address.get(c), secure);
                        	Log.e("CHeck", "Connect");
                            if(mAutoConnectionListener != null)
